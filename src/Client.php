...
class Client {
    ...
    public function connect() {
        try {
//          GuzzleHttp\RequestOptions::VERIFY => false : Validation des certificats désactivée
            $response = $this->http->request ( 'POST', $this->url . '/oauth2/token/', array (GuzzleHttp\RequestOptions::VERIFY => false, 'form_params' => array ('client_id' => $this->client_id,'client_secret' => $this->client_secret,'username' => $this->username,'password' => $this->password,'redirect_uri' => 'http://localhost', // $this->redirect_uri,
'grant_type' => 'password','scope' => implode ( ' ', $this->scope )
            )
            ) );
    ...
    private function callAPI($path, $method = 'GET', $opts = [], $raw = false) {
        try {
            $p = [ 'headers' => [ 'Authorization' => 'Bearer ' . $this->access_token
            ]
            ];
            $opts[GuzzleHttp\RequestOptions::VERIFY] = false;
            if (sizeof ( $opts ) > 0) {
    ...
    public function uploadFile($params, $fileSource) {
        $opts = [ 'multipart' => [ [ 'name' => 'file','filename' => 'filename.txt','contents' => $fileSource
        ]
        ]
        ];
